...
@Component
public class CustomerEmailFilter implements FieldFilter<CustomerFilter> {
    @Override
    public boolean apply(CustomerFilter customerFilter) {
        return Objects.nonNull(customerFilter.getEmail());
    }

    @Override
    public void addSearchCriteria(CustomerFilter customerFilter, List<SearchCriteria> searchCriteriaList) {
        searchCriteriaList.add(SearchCriteria.builder().key("email").operation(SearchOperation.LIKE).value(customerFilter.getEmail()).build());
    }
}
